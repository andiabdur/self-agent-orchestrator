...
*   **Run in background**: Execute `launcher.vbs` (which is generated by the installer to run Node silently in the background).
*   **Stop**: Close the terminal window if running in foreground, or end the `node.exe` process via Task Manager if running in background.
